...
version 4.0, developed by the Jakarta EL Team under the Eclipse
...
`IntegerLiteral` construct in link:ELSpec.html#a524[See
Collected Syntax]

* Floating point - As defined by the
`FloatingPointLiteral` construct in link:ELSpec.html#a524[See
Collected Syntax]
...
** If prop-a is an EL variable (see link:ELSpec.html#a393[See Variables]),
...
{empty}link:ELSpec.html#a1155[See predicate]
...
{empty}link:ELSpec.html#a1158[See mapper]
...
{empty}link:ELSpec.html#a1161[See comparator]
...
{empty}link:ELSpec.html#a1164[See consumer]
...
Exception classes that extend `ELException` (
`PropertyNotFoundException`, `PropertyNotWritableException`,
`MethodNotFoundException` ) did not have a constructor with both '
...
** Creation methods now use `ELContex` t instead
of `FunctionMapper` (see EL Variables above).

** Added method `coerceToType().` See issue 132 at
